...
// GetAccounts takes any number of account names and performs the needed actions to return a
// pointer to `Account` (the underlying type of `Account` is a struct).
//
//Example:
//import (
//	"fmt"
//
//	"github.com/jrswab/go-hive/accounts"
//)
//
//func main() {
//	c := accounts.NewJRPC()
//	accStruct, err := c.GetAccounts("jrswab")
//	if err != nil {
//		// handle  error
//	}
//	fmt.Println(accStruct.Data[0].Name) // Prints out the first account name that was passed in.
//}
func (jrpc *JSONrpc) GetAccounts(account ...string) (*Accounts, error) {
	prep := make([]string, 0)
	for _, name := range account {
		prep = append(prep, name)
	}
	params := [][]string{prep}
...
func (jrpc *JSONrpc) GetAccountHistory(account string, start, total int) {
	...
	*/
	...
	params := fmt.Sprint("[\"%s\", %d, %d]", account, start, total)

	jrpc.Method = "get_account_history"
	jrpc.Params = params
	raw, err := jrpc.getAccountData()
	if err != nil {
		fmt.Println(err)
